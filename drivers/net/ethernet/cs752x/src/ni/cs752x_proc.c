...
#endif
	proc_ni = proc_create(CS752x_NI_PROC_NAME, S_IRUGO, init_net.proc_net, &ni_file_ops);
	if (!proc_ni) goto init_bad;
	proc_sch = proc_create(CS752x_SCH_PROC_NAME, S_IRUGO, init_net.proc_net, &sch_file_ops);
	if (!proc_sch) goto init_bad;
	proc_dma = proc_create(CS752x_DMA_PROC_NAME, S_IRUGO, init_net.proc_net, &dma_file_ops);
	if (!proc_dma) goto init_bad;
	proc_mdio = proc_create(CS752x_MDIO_PROC_NAME, S_IRUGO, init_net.proc_net, &mdio_file_ops);
	if (!proc_mdio) goto init_bad;
	proc_qm = proc_create(CS752x_QM_PROC_NAME, S_IRUGO, init_net.proc_net, &qm_file_ops);
	if (!proc_qm) goto init_bad;
	proc_fe = proc_create(CS752x_FE_PROC_NAME, S_IRUGO, init_net.proc_net, &fe_file_ops);
	if (!proc_fe) goto init_bad;

	proc_tm = proc_create(CS752x_TM_PROC_NAME, S_IRUGO, init_net.proc_net, &tm_file_ops);
...
init_bad:
	if (proc_ni) remove_proc_entry(CS752x_NI_PROC_NAME, proc_ni);
	if (proc_qm) remove_proc_entry(CS752x_QM_PROC_NAME, proc_qm);
	if (proc_sch) remove_proc_entry(CS752x_SCH_PROC_NAME, proc_sch);
	if (proc_dma) remove_proc_entry(CS752x_DMA_PROC_NAME, proc_dma);
	if (proc_mdio) remove_proc_entry(CS752x_MDIO_PROC_NAME, proc_mdio);
	if (proc_fe) remove_proc_entry(CS752x_FE_PROC_NAME, proc_fe);
	if (proc_tm) remove_proc_entry(CS752x_TM_PROC_NAME, proc_tm);
...
{
	remove_proc_entry(CS752x_NI_PROC_NAME, proc_ni);
	remove_proc_entry(CS752x_QM_PROC_NAME, proc_qm);
	remove_proc_entry(CS752x_DMA_PROC_NAME, proc_dma);
	remove_proc_entry(CS752x_SCH_PROC_NAME, proc_sch);
	remove_proc_entry(CS752x_MDIO_PROC_NAME, proc_mdio);
	remove_proc_entry(CS752x_FE_PROC_NAME, proc_fe);
	remove_proc_entry(CS752x_TM_PROC_NAME, proc_tm);
