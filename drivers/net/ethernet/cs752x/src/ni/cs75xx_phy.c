...
void cs_ni_phy_start(mac_info_t *tp)
{
	...
	phydev = phy_find_first(tp->mdio_bus);
...
void cs_ni_phy_stop(mac_info_t *tp)
{
	...
	phy_stop(phy_find_first(tp->mdio_bus));
}
...
static void cs_ni_adjust_link(struct net_device *dev)
{
	...
	struct phy_device *phydev = phy_find_first(tp->mdio_bus);
	//unsigned long flags;
	int status_change = 0;
...
int cs_phy_init(mac_info_t *tp)
{
	...
	printk(KERN_INFO "GE-%d: attached PHY driver [%s] "
		...
		tp->port_id, phydev->drv->name, phydev_name(phydev),
		phydev->phy_id);
	...
	case PHY_INTERFACE_MODE_MII:
		...
		break;
	default:
		phy_disconnect(phy_find_first(tp->mdio_bus));
		return -EINVAL;
	}
