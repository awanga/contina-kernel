...
void cs_ni_instance_buf_low_clear()
{
	int i;
...
/* 3.4.11 Change for register as platform device */
static int __devinit cs_ni_init_module_probe(struct platform_device *pdev)
{
	int i, err = 0;
	mac_info_t *tp;
	struct net_device *dev;
...
static struct platform_driver cs_ni_platform_driver = {
	...
	.remove		= __devexit_p(cs_ni_cleanup_module_exit),
	.driver = {
		.name		= "g2-ne",
