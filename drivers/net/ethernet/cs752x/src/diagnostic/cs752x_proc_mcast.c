 ...
 */
static int cs_proc_l2_mcast_member_add_write_proc(struct file *file, const char __user *buffer,
 ...
 */
static int cs_proc_l2_mcast_member_del_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *off)
{
	char buf[256];
	cs_port_id_t dev_id;
...
static int cs_l2_mcast_port_member_get_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *off)
{
	char buf[32];
	cs_port_id_t port;
	...
	buf[len] = '\0';
	if (kstrtoul(buf, 0, (long unsigned int*) &port))
		goto MCS_L2_MCAST_PORT_MEMBER_CLEAR_INVAL_EXIT;
...
static int cs_l2_mcast_port_member_clear_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *off)
{
	char buf[32];
	unsigned long port;
...
static int cs_l2_mcast_member_all_clear_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *off)
{
	char buf[32];
	unsigned long mask;
...
static int cs_l2_mcast_mode_set_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *off)
{
	char buf[32];
	unsigned long port;
...
static int cs_l2_mcast_wan_port_set_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *off)
{
	char buf[32];
	unsigned long port;
...
static int cs_l2_mcast_wan_port_set_read_proc(struct seq_file *m, void *v)
{
	u32 wan_port_id;
...
#define CS752X_DEFINE_WO_PROC_OPS(x) \
	_CS752X_DEFINE_PROC_OPS(x, NULL, x##_write_proc)

#define CS752X_DEFINE_RW_PROC_OPS(x) \
	_CS752X_DEFINE_PROC_OPS(x, x##_read_proc, x##_write_proc)

CS752X_DEFINE_WO_PROC_OPS(cs_proc_l2_mcast_member_add)
CS752X_DEFINE_WO_PROC_OPS(cs_proc_l2_mcast_member_del)
CS752X_DEFINE_WO_PROC_OPS(cs_l2_mcast_port_member_get)
CS752X_DEFINE_WO_PROC_OPS(cs_l2_mcast_port_member_clear)
CS752X_DEFINE_WO_PROC_OPS(cs_l2_mcast_member_all_clear)
CS752X_DEFINE_WO_PROC_OPS(cs_l2_mcast_mode_set)
CS752X_DEFINE_RW_PROC_OPS(cs_l2_mcast_wan_port_set)

void cs_mcast_proc_init_module(void)
{

	cs752x_add_proc_handler(CS_L2_MCAST_MEMBER_ADD,
				&cs_proc_l2_mcast_member_add_proc_fops,
				proc_driver_cs752x_mcast);

	cs752x_add_proc_handler(CS_L2_MCAST_MEMBER_DEL,
				&cs_proc_l2_mcast_member_del_proc_fops,
				proc_driver_cs752x_mcast);

	cs752x_add_proc_handler(CS_L2_MCAST_PORT_MEMBER_GET,
				&cs_l2_mcast_port_member_get_proc_fops,
				proc_driver_cs752x_mcast);

	cs752x_add_proc_handler(CS_L2_MCAST_PORT_MEMBER_CLEAR,
				&cs_l2_mcast_port_member_clear_proc_fops,
				proc_driver_cs752x_mcast);

	cs752x_add_proc_handler(CS_L2_MCAST_MEMBER_ALL_CLEAR,
				&cs_l2_mcast_member_all_clear_proc_fops,
				proc_driver_cs752x_mcast);

	cs752x_add_proc_handler(CS_L2_MCAST_MODE_SET,
				&cs_l2_mcast_mode_set_proc_fops,
				proc_driver_cs752x_mcast);

	cs752x_add_proc_handler(CS_L2_MCAST_WAN_PORT_SET,
				&cs_l2_mcast_wan_port_set_proc_fops,
				proc_driver_cs752x_mcast);
