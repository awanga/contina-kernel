...
#define CS752X_DEFINE_RO_PROC_OPS(x) _CS752X_DEFINE_PROC_OPS(x, x##_read_proc, NULL)
#define CS752X_DEFINE_RW_PROC_OPS(x) _CS752X_DEFINE_PROC_OPS(x, x##_read_proc, x##_write_proc)
...
int cs752x_add_proc_handler(const char *name,
			    const struct file_operations *proc_fops,
			    struct proc_dir_entry *parent)
{
	struct proc_dir_entry *node;

	node = proc_create(name, S_IRUGO | S_IWUGO, parent, proc_fops);
	if (!node) {
...
static int cs_pcie_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, PCIE_DEBUG, PCIE_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", PCIE_DEBUG, cs_pcie_debug);
	return 0;
}

static int cs_pcie_debug_write_proc(struct file *file, const char __user *buffer,
...
static int cs_sata_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, SATA_DEBUG, SATA_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", SATA_DEBUG, cs_sata_debug);
	return 0;
}

static int cs_sata_debug_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_acp_enable_read_proc(struct seq_file *m, void *v)
{
	if((system_rev & 0xFF) != 0xA1){
		seq_printf(m, "ACP not supported in A0 Chip\n");
		return 0;
	}

	seq_printf(m, CS752X_ACP_ENABLE_HELP_MSG, ACP_ENABLE, ACP_ENABLE);
	seq_printf(m, "\n%s = 0x%08x\n", ACP_ENABLE, cs_acp_enable);
	return 0;
}

static int cs_acp_enable_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_sd_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, SD_DEBUG, SD_DEBUG);
...
static int cs_wfo_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_WFO_DEBUG_MSG, WFO_DEBUG, WFO_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", WFO_DEBUG, cs_wfo_debug);
	return 0;
}

static int cs_wfo_debug_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_wfo_enable_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_WFO_ENABLE_MSG, WFO_ENABLE, WFO_ENABLE);
	seq_printf(m, "\n%s = 0x%08x\n", WFO_ENABLE, cs_wfo_enable);
	return 0;
}

static int cs_wfo_enable_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_wfo_csme_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_WFO_CSME_MSG, WFO_CSME, WFO_CSME);
	seq_printf(m, "\n%s = 0x%08x\n", WFO_CSME, cs_wfo_csme);
	return 0;
}

static int cs_wfo_csme_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_wfo_get_pe0_mibs_read_proc(struct seq_file *m, void *v)
{
...
static int cs_wfo_get_pe0_mibs_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	cs_mib_pe_s mib_counter;
...
static int cs_wfo_get_pe1_mibs_read_proc(struct seq_file *m, void *v)
{
	cs_mib_pe_s mib_counter;
...
static int cs_wfo_get_pe1_mibs_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	cs_mib_pe_s mib_counter;
...
static int cs_wfo_get_pe0_logs_read_proc(struct seq_file *m, void *v)
{
	char logs[512];
...
static int cs_wfo_get_pe1_logs_read_proc(struct seq_file *m, void *v)
{
	char logs[512];
...
static int cs_wfo_get_pe0_fwd_tbl_read_proc(struct seq_file *m, void *v)
{
...
static int cs_wfo_get_pe1_fwd_tbl_read_proc(struct seq_file *m, void *v)
{
...
static int cs_wfo_rate_adjust_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, "Purpose: Enable Wi-Fi Offload Rate Adjustment");
	seq_printf(m, "\n%s = 0x%08x\n", WFO_RATE_ADJUST, cs_wfo_rate_adjust);
	return 0;
}

static int cs_wfo_rate_adjust_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_wfo_rate_adjust_period_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, "Purpose: Read Wi-Fi Offload Rate Adjustment Timer Period");
	seq_printf(m, "\n%s = %dms\n", WFO_RATE_ADJUST_PERIOD, cs_wfo_rate_adjust_period);
	return 0;
}

static int cs_wfo_rate_adjust_period_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_sd_debug_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_spdif_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, SPDIF_DEBUG, SPDIF_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", SPDIF_DEBUG,
			cs_spdif_debug);
	return 0;
}

static int cs_spdif_debug_write_proc(struct file *file, const char __user *buffer,
				     size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_spi_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, SPI_DEBUG, SPI_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", SPI_DEBUG, cs_spi_debug);
	return 0;
}

static int cs_spi_debug_write_proc(struct file *file, const char __user *buffer,
				     size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ssp_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, SSP_DEBUG, SSP_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", SSP_DEBUG, cs_ssp_debug);
	return 0;
}

static int cs_ssp_debug_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ts_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, TS_DEBUG, TS_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", TS_DEBUG, cs_ts_debug);
	return 0;
}

static int cs_ts_debug_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_usb_host_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, USB_HOST_DEBUG,
			USB_HOST_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", USB_HOST_DEBUG,
			cs_usb_host_debug);
	return 0;
...
static int cs_usb_host_debug_write_proc(struct file *file, const char __user *buffer,
					size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_usb_dev_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, USB_DEV_DEBUG,
			USB_DEV_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", USB_DEV_DEBUG,
			cs_usb_dev_debug);
	return 0;
...
static int cs_usb_dev_debug_write_proc(struct file *file, const char __user *buffer,
				       size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_fb_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, FB_DEBUG, FB_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", FB_DEBUG, cs_fb_debug);
	return 0;
}

static int cs_fb_debug_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_crypto_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, CRYPTO_DEBUG, CRYPTO_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", CRYPTO_DEBUG,
			cs_crypto_debug);
	return 0;
}

static int cs_crypto_debug_write_proc(struct file *file, const char __user *buffer,
				      size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_pwr_ctrl_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, PWR_CTRL_DEBUG,
			PWR_CTRL_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", PWR_CTRL_DEBUG,
			cs_pwr_ctrl_debug);
	return 0;
...
static int cs_pwr_ctrl_debug_write_proc(struct file *file, const char __user *buffer,
					size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_cir_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, CIR_DEBUG, CIR_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", CIR_DEBUG, cs_cir_debug);
	return 0;
}

static int cs_cir_debug_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_rt3593_noncache_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, RT3593_NONCACHE, RT3593_NONCACHE);
	seq_printf(m, "\n%s = 0x%08x\n", RT3593_NONCACHE, cs_rt3593_noncache);
	return 0;
}

static int cs_rt3593_noncache_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_rt3593_dma_sync_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, RT3593_DMA_SYNC, RT3593_DMA_SYNC);
	seq_printf(m, "\n%s = 0x%08x\n", RT3593_DMA_SYNC, cs_rt3593_dma_sync);
	return 0;
}

static int cs_rt3593_dma_sync_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_rt3593_tx_lock_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, RT3593_TX_LOCK, RT3593_TX_LOCK);
	seq_printf(m, "\n%s = 0x%08x\n", RT3593_TX_LOCK, cs_rt3593_tx_lock);
	return 0;
}

static int cs_rt3593_tx_lock_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ipc_list_status_read_proc(struct seq_file *m, void *v)
{
...
static int cs_ipc_list_status_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *pos)
{
	/* FIXME! Move IPC debug proc to IPC module */
#ifdef CONFIG_CS75xx_IPC2RCPU
...
static int cs_ni_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_NI_HELP_MSG, NI_DEBUG, NI_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", NI_DEBUG, cs_ni_debug);
	return 0;
}

static int cs_ni_debug_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ni_use_sendfile_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NI_USE_SENDFILE_HELP_MSG,
...
static int cs_ni_use_sendfile_write_proc(struct file *file, const char __user *buffer,
		size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ni_fastbridge_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG,
...
static int cs_ni_fastbridge_write_proc(struct file *file, const char __user *buffer,
		size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ni_clear_stats_read_proc(struct seq_file *m, void *v)
{
        seq_printf(m, CS752X_NI_CLEAR_STATS_HELP_MSG,
...
static int cs_ni_clear_stats_write_proc(struct file *file, const char __user *buffer,
                size_t count, loff_t *pos)
{
        char buf[32];
        unsigned long mask;
...
static int cs_ni_min_rsve_mem_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, "\n%s = 0x%08x\n", NI_MIN_RSVE_MEM, cs_ni_min_rsve_mem);
	return 0;
}

static int cs_ni_min_rsve_mem_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ni_delay_ms_rsve_mem_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, "\n%s = 0x%08x\n", NI_DELAY_MS_RSVE_MEM, cs_ni_delay_ms_rsve_mem);
	return 0;
}

static int cs_ni_delay_ms_rsve_mem_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
extern int cs_ni_instance_buf_low_clear(void);

static int cs_ni_instance_buf_low_read_proc(struct seq_file *m, void *v)
{
	int i;
	int j;
	for (i=0; (j = cs_ni_instance_buf_low_read(i)) >= 0; ++i) {
		seq_printf(m, "instance %d = 0x%08x\n", i, j);
	}
	return 0;
}

static int cs_ni_instance_buf_low_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
...
static int cs_ni_skb_recycle_max_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", cs_ni_skb_recycle_max);
	return 0;
}

static int cs_ni_skb_recycle_max_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long val;
...
extern int cs_ni_skb_recycle_len_cpu0(void);
extern int cs_ni_skb_recycle_len_cpu1(void);

static int cs_ni_skb_recycle_queue_read_proc(struct seq_file *m, void *v)
{
...
static int cs_ne_cs_cb_debug_read_proc(struct seq_file *m, void *v)
{
//	seq_printf(m, CS752X_CORE_CS_CB_DEBUG_MESG,
//			CORE_CS_CB_DEBUG);
	cs_accel_cb_print();
	return 0;
}
...
static int cs_qm_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, QM_DEBUG, QM_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", QM_DEBUG, cs_qm_debug);
	return 0;
}

static int cs_qm_debug_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_qm_int_buff_read_proc(struct seq_file *m, void *v)
{
    switch (internal_buff_size) {
    	case CS_QM_INT_BUFF_0:
    		seq_printf(m, "Use external DDR for QM\n");
    	    break;
    	case CS_QM_INT_BUFF_128:
			seq_printf(m, "Half Size Internal Buffers ON - 128KB Packet\n");
			seq_printf(m, " - 128kB free\n");
    	    break;
    	case CS_QM_INT_BUFF_192:
			seq_printf(m, "Half Size Internal Buffers ON - 192KB Packet\n");
			seq_printf(m, " - 64kB free\n");
    	    break;
    	case CS_QM_INT_BUFF_256:
		    seq_printf(m, "Full Size Internal Buffers ON - 256KB Packet\n");
    	    break;
    }
...
static int cs_tm_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, TM_DEBUG, TM_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", TM_DEBUG, cs_tm_debug);
	return 0;
}

static int cs_tm_debug_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_sch_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, SCH_DEBUG, SCH_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", SCH_DEBUG, cs_sch_debug);
	return 0;
}

static int cs_sch_debug_write_proc(struct file *file, const char __user *buffer,
				   size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_switch_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_HELP_MSG, SWITCH_DEBUG, SWITCH_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", SWITCH_DEBUG,
			cs_switch_debug);
	return 0;
}

static int cs_switch_debug_write_proc(struct file *file, const char __user *buffer,
				      size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_fe_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_FE_HELP_MSG, FE_DEBUG, FE_DEBUG);
...
static int cs_fe_debug_write_proc(struct file *file, const char __user *buffer,
				  size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_fe_double_check_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_FE_DOUBLE_CHECK_HELP_MSG,
...
static int cs_adapt_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_ADAPT_HELP_MSG, ADAPT_DEBUG,
			ADAPT_DEBUG);
	seq_printf(m, "\n%s = 0x%08x\n", ADAPT_DEBUG,
			cs_adapt_debug);
	return 0;
...
static int cs_adapt_debug_write_proc(struct file *file, const char __user *buffer,
				     size_t count, loff_t *pos)
{
	char buf[32];
	unsigned long mask;
...
static int cs_ne_core_logic_debug_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_CORE_LOGIC_HELP_MSG,
 ...
 */
static int cs_switch_reg_read_proc(struct seq_file *m, void *v)
{
	seq_printf(m, REG_HELP_MSG, SWITCH_REG, SWITCH_REG,
...
static int cs_switch_reg_write_proc(struct file *file, const char __user *buffer,
				    size_t count, loff_t *pos)
{
...
CS752X_DEFINE_RW_PROC_OPS(cs_ni_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_ni_use_sendfile)
CS752X_DEFINE_RW_PROC_OPS(cs_ni_fastbridge)
CS752X_DEFINE_RW_PROC_OPS(cs_ni_clear_stats)
CS752X_DEFINE_RW_PROC_OPS(cs_ni_min_rsve_mem)
CS752X_DEFINE_RW_PROC_OPS(cs_ni_delay_ms_rsve_mem)
CS752X_DEFINE_RW_PROC_OPS(cs_ni_instance_buf_low)
#ifdef CONFIG_CS75XX_MUT
CS752X_DEFINE_RW_PROC_OPS(mut_skb_recycle)
CS752X_DEFINE_RW_PROC_OPS(mut_ne_memtrace)
#endif
#ifdef CONFIG_CS75XX_NI_EXPERIMENTAL_SW_CACHE_MANAGEMENT
CS752X_DEFINE_RW_PROC_OPS(cs_ni_skb_recycle_max)
#endif
CS752X_DEFINE_RO_PROC_OPS(cs_ni_skb_recycle_queue)
CS752X_DEFINE_RW_PROC_OPS(cs_qm_debug)
CS752X_DEFINE_RO_PROC_OPS(cs_qm_int_buff)
CS752X_DEFINE_RW_PROC_OPS(cs_tm_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_sch_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_switch_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_switch_reg)
CS752X_DEFINE_RW_PROC_OPS(cs_fe_debug)
#if defined(CONFIG_CS75XX_DOUBLE_CHECK)
CS752X_DEFINE_RW_PROC_OPS(cs_fe_double_check)
#endif
CS752X_DEFINE_RW_PROC_OPS(cs_adapt_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_ne_core_logic_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_ne_cs_cb_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_pcie_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_sata_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_acp_enable)
#ifdef CONFIG_CS75XX_WFO
CS752X_DEFINE_RW_PROC_OPS(cs_wfo_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_wfo_enable)
CS752X_DEFINE_RW_PROC_OPS(cs_wfo_csme)
CS752X_DEFINE_RW_PROC_OPS(cs_wfo_get_pe0_mibs)
CS752X_DEFINE_RW_PROC_OPS(cs_wfo_get_pe1_mibs)

CS752X_DEFINE_RO_PROC_OPS(cs_wfo_get_pe0_logs)
CS752X_DEFINE_RO_PROC_OPS(cs_wfo_get_pe1_logs)
CS752X_DEFINE_RO_PROC_OPS(cs_wfo_get_pe0_fwd_tbl)
CS752X_DEFINE_RO_PROC_OPS(cs_wfo_get_pe1_fwd_tbl)
CS752X_DEFINE_RW_PROC_OPS(cs_wfo_rate_adjust)
CS752X_DEFINE_RW_PROC_OPS(cs_wfo_rate_adjust_period)
#endif
CS752X_DEFINE_RW_PROC_OPS(cs_sd_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_spdif_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_spi_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_ssp_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_ts_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_usb_host_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_usb_dev_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_fb_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_crypto_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_pwr_ctrl_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_cir_debug)
CS752X_DEFINE_RW_PROC_OPS(cs_rt3593_noncache)
CS752X_DEFINE_RW_PROC_OPS(cs_rt3593_tx_lock)
CS752X_DEFINE_RW_PROC_OPS(cs_rt3593_dma_sync)
CS752X_DEFINE_RW_PROC_OPS(cs_ipc_list_status)
...
//-- For WFO
#endif
	cs752x_add_proc_handler(NI_DEBUG,
				&cs_ni_debug_proc_fops,
				proc_driver_cs752x_ne_ni);
	cs752x_add_proc_handler(NI_USE_SENDFILE,
				&cs_ni_use_sendfile_proc_fops,
				proc_driver_cs752x_ne_ni);
	cs752x_add_proc_handler(NI_FASTBRIDGE,
				&cs_ni_fastbridge_proc_fops,
				proc_driver_cs752x_ne_ni);
	cs752x_add_proc_handler(NI_CLEAR_STATS,
                                &cs_ni_clear_stats_proc_fops,
                                proc_driver_cs752x_ne_ni);
	cs752x_add_proc_handler(NI_MIN_RSVE_MEM,
                                &cs_ni_min_rsve_mem_proc_fops,
                                proc_driver_cs752x_ne_ni);
	cs752x_add_proc_handler(NI_DELAY_MS_RSVE_MEM,
                                &cs_ni_delay_ms_rsve_mem_proc_fops,
                                proc_driver_cs752x_ne_ni);
	cs752x_add_proc_handler(NI_INSTANCE_BUF_LOW,
                                &cs_ni_instance_buf_low_proc_fops,
                                proc_driver_cs752x_ne_ni);
#ifdef CONFIG_CS75XX_MUT
	cs752x_add_proc_handler(SKB_RECYCLE,
                                &mut_skb_recycle_proc_fops,
                                proc_driver_cs752x_ne_ni);
#endif
#ifdef CONFIG_CS75XX_NI_EXPERIMENTAL_SW_CACHE_MANAGEMENT
	cs752x_add_proc_handler(SKB_RECYCLE_MAX,
                                &cs_ni_skb_recycle_max_proc_fops,
                                proc_driver_cs752x_ne_ni);
#endif
	cs752x_add_proc_handler(SKB_RECYCLE_QUEUE_LEN,
				&cs_ni_skb_recycle_queue_proc_fops,
                                proc_driver_cs752x_ne_ni);
	cs752x_add_proc_handler(QM_DEBUG,
				&cs_qm_debug_proc_fops,
				proc_driver_cs752x_ne_qm);
	cs752x_add_proc_handler(QM_INT_BUFF,
				&cs_qm_int_buff_proc_fops,
				proc_driver_cs752x_ne_qm);
	cs752x_add_proc_handler(TM_DEBUG,
				&cs_tm_debug_proc_fops,
				proc_driver_cs752x_ne_tm);
	cs752x_add_proc_handler(SCH_DEBUG,
				&cs_sch_debug_proc_fops,
				proc_driver_cs752x_ne_sch);
	cs752x_add_proc_handler(SWITCH_DEBUG,
				&cs_switch_debug_proc_fops,
				proc_driver_cs752x_ne_switch);
	cs752x_add_proc_handler(SWITCH_REG,
				&cs_switch_reg_proc_fops,
				proc_driver_cs752x_ne_switch);
	cs752x_add_proc_handler(FE_DEBUG,
				&cs_fe_debug_proc_fops,
				proc_driver_cs752x_ne_fe);
#if defined(CONFIG_CS75XX_DOUBLE_CHECK)
	cs752x_add_proc_handler(FE_DOUBLE_CHECK,
				&cs_fe_double_check_proc_fops,
				proc_driver_cs752x_ne_fe);
#endif

	cs752x_add_proc_handler(ADAPT_DEBUG,
				&cs_adapt_debug_proc_fops,
				proc_driver_cs752x_ne_adaptation);

	cs752x_add_proc_handler(CORE_LOGIC_DEBUG,
				&cs_ne_core_logic_debug_proc_fops,
				proc_driver_cs752x_ne_core);

	cs752x_add_proc_handler(CORE_CS_CB_DEBUG,
					&cs_ne_cs_cb_debug_proc_fops,
					proc_driver_cs752x_ne_core);

#ifdef CONFIG_CS75XX_MUT
	cs752x_add_proc_handler(NE_MEMTRACE,
					&mut_ne_memtrace_proc_fops,
					proc_driver_cs752x_ne_core);
#endif

	cs752x_add_proc_handler(PCIE_DEBUG,
				&cs_pcie_debug_proc_fops,
				proc_driver_cs752x_pcie);
	cs752x_add_proc_handler(SATA_DEBUG,
				&cs_sata_debug_proc_fops,
				proc_driver_cs752x_sata);
	cs752x_add_proc_handler(ACP_ENABLE,
				&cs_acp_enable_proc_fops,
				proc_driver_cs752x_acp);
#ifdef CONFIG_CS75XX_WFO
//++ For WFO
	cs752x_add_proc_handler(WFO_DEBUG,
				&cs_wfo_debug_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_ENABLE,
				&cs_wfo_enable_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_CSME,
				&cs_wfo_csme_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_GET_PE0_MIBS,
				&cs_wfo_get_pe0_mibs_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_GET_PE1_MIBS,
				&cs_wfo_get_pe1_mibs_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_GET_PE0_LOGS,
				&cs_wfo_get_pe0_logs_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_GET_PE1_LOGS,
				&cs_wfo_get_pe1_logs_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_PE0_FWD_TBL,
				&cs_wfo_get_pe0_fwd_tbl_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_PE1_FWD_TBL,
				&cs_wfo_get_pe1_fwd_tbl_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_RATE_ADJUST,
				&cs_wfo_rate_adjust_proc_fops,
				proc_driver_cs752x_wfo);
	cs752x_add_proc_handler(WFO_RATE_ADJUST_PERIOD,
				&cs_wfo_rate_adjust_period_proc_fops,
				proc_driver_cs752x_wfo);
//-- For WFO
#endif
	cs752x_add_proc_handler(SD_DEBUG,
				&cs_sd_debug_proc_fops, proc_driver_cs752x_sd);
	cs752x_add_proc_handler(SPDIF_DEBUG,
				&cs_spdif_debug_proc_fops,
				proc_driver_cs752x_spdif);
	cs752x_add_proc_handler(SPI_DEBUG,
				&cs_spi_debug_proc_fops,
				proc_driver_cs752x_spi);
	cs752x_add_proc_handler(SSP_DEBUG,
				&cs_ssp_debug_proc_fops,
				proc_driver_cs752x_ssp);
	cs752x_add_proc_handler(TS_DEBUG,
				&cs_ts_debug_proc_fops, proc_driver_cs752x_ts);
	cs752x_add_proc_handler(USB_HOST_DEBUG,
				&cs_usb_host_debug_proc_fops,
				proc_driver_cs752x_usb_host);
	cs752x_add_proc_handler(USB_DEV_DEBUG,
				&cs_usb_dev_debug_proc_fops,
				proc_driver_cs752x_usb_dev);
	cs752x_add_proc_handler(FB_DEBUG,
				&cs_fb_debug_proc_fops, proc_driver_cs752x_fb);
	cs752x_add_proc_handler(CRYPTO_DEBUG,
				&cs_crypto_debug_proc_fops,
				proc_driver_cs752x_crypto);
	cs752x_add_proc_handler(PWR_CTRL_DEBUG,
				&cs_pwr_ctrl_debug_proc_fops,
				proc_driver_cs752x_pwr_ctrl);
	cs752x_add_proc_handler(CIR_DEBUG,
				&cs_cir_debug_proc_fops,
				proc_driver_cs752x_cir);
	cs752x_add_proc_handler(RT3593_NONCACHE,
				&cs_rt3593_noncache_proc_fops,
				proc_driver_cs752x_rt3593);
	cs752x_add_proc_handler(RT3593_TX_LOCK,
				&cs_rt3593_tx_lock_proc_fops,
				proc_driver_cs752x_rt3593);
	cs752x_add_proc_handler(RT3593_DMA_SYNC,
				&cs_rt3593_dma_sync_proc_fops,
				proc_driver_cs752x_rt3593);
	cs752x_add_proc_handler(IPC_LIST_STATUS,
				&cs_ipc_list_status_proc_fops,
				proc_driver_cs752x_ipc);
