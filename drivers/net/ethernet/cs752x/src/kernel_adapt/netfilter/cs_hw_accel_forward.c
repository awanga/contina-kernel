	...
	if (ret != 0) {
		...
		if (time_before(expire_time, jiffies)) {
			...
			if (!nf_ct_is_confirmed(ct)) {
				ct->timeout = nfct_time_stamp + extra_jiffies;
			    ...
			    */
				...
				DBG(printk("\t now %ld  - expired %ld  = %ld\n",
					jiffies, ct->timeout, jiffies - ct->timeout));
	...
	for (cur_range_net = &ct_range_net_head; cur_range_net != NULL; cur_range_net = ct_range_net_head.next) {
		if (cur_range_net->net != NULL) {
			for (i = 0; i < nf_conntrack_htable_size; ++i) {
				matched = 0;
				hlist_nulls_for_each_entry_rcu(h, n, &nf_conntrack_hash[i], hnnode) {
