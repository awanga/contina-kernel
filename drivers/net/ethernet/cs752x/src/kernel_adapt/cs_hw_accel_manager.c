...
static int cs_proc_node_hash_timer_period_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_HASH_TIMER_PERIOD_HELP_MSG,
...
static int cs_proc_node_hash_default_lifetime_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_HASH_DEFAULT_LIFETIME_HELP_MSG,
...
static int cs_proc_node_kernel_mod_hw_accel_enable_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_ADAPT_ENABLE_HELP_MSG,
...
static int cs_proc_node_allow_localin_port_read(struct seq_file *m, void *v)
{
...
static int cs_proc_acl_add_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n",
			acl_indx);
	return 0;
...
static int cs_proc_acl_add_write(struct file *file, const char __user *buffer,
	...
	if ((pch = strstr(buf, " ")) != NULL) {
			...
			*pch = '\0';
			...
			if (action == 0) {
				...
				if (kstrtoul(pch2, 0, &entry.ip_da[0]))
				{
					printk("%s %d \n", __func__, __LINE__);
					return count;
				}
				...
				if (kstrtoul(pch, 0, &entry.ip_da[1]))
				{
					printk("%s %d \n", __func__, __LINE__);
					return count;
				}
				...
				if (kstrtoul(pch2, 0, &entry.ip_da[2]))
				{
					printk("%s %d \n", __func__, __LINE__);
					return count;
				}
				...
				if (kstrtoul(pch, 0, &entry.ip_da[3]))
				{
					printk("%s %d \n", __func__, __LINE__);
					return count;
				}
...
static int cs_proc_node_localin_udp_drop_statistic_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_tcp_bypass_port_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_udp_bypass_port_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_expected_bypass_port_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_tcp_priority_port_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_udp_priority_port_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_nf_drop_check_try_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_NF_DROP_CHECK_TRY_HELP_MSG,
...
static int cs_proc_node_nf_drop_check_life_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_NF_DROP_CHECK_LIFE_HELP_MSG,
...
static int cs_proc_node_nf_drop_check_table_read(struct seq_file *m, void *v)
{
...
#ifdef CONFIG_CS752X_HW_ACCELERATION_IPSEC
static int cs_proc_node_ipsec_sadb_table_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_ipsec_pe_sadb_table_read(struct seq_file *m, void *v)
{
...
#ifdef CONFIG_CS752X_FASTNET
static int cs_proc_node_kernel_mod_fastnet_enable_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_ADAPT_FASTNET_ENABLE_HELP_MSG,
...
static int cs_proc_node_hash_hw_entry_read(struct seq_file *m, void *v)
{
...
static int cs_proc_node_core_hmu_tbl_read(struct seq_file *m, void *v)
{
	if (hmu_table_num != 0)
...
static int cs_proc_node_core_vtbl_read(struct seq_file *m, void *v)
{
	cs_core_vtable_dump();
	return 0;
}
...
static int cs_proc_node_ipsec_offload_mode_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_IPSEC_OFFLOAD_MODE_HELP_MSG,
...
#ifdef CONFIG_CS75XX_HW_ACCEL_IPLIP
static int cs_proc_node_iplip_pe_cmd_read(struct seq_file *m, void *v)
{
	seq_printf(m, "IPv6 over PPP over L2TP over IPv4 over PPPoE"
		...
		const char __user *buffer, size_t count, loff_t *off)
{
	...
	if (kstrtoul(token_list[0], 0, &cmd) ||
	...
	case CS_IPLIP_IPC_PE_SET_ENTRY:
		if (kstrtoul(token_list[1], 0, &index) ||
	...
	case CS_IPLIP_IPC_PE_MIB_EN:
		if (kstrtoul(token_list[1], 0, &enbl) || enbl > 1)
			goto IPLIP_INVAL_EXIT;
...
static int cs_proc_node_iplip_kernel_cmd_read(struct seq_file *m, void *v)
{
	seq_printf(m, "IPv6 over PPP over L2TP over IPv4 over PPPoE"
		...
		const char __user *buffer, size_t count, loff_t *off)
{
	...
	if (kstrtoul(token_list[1], 0, &pppoe_port))
		goto IPLIP_INVAL_EXIT;

	if (kstrtoul(token_list[2], 0, &ifindex))
		goto IPLIP_INVAL_EXIT;
...
IPLIP_INVAL_EXIT:
	...
	defined(CONFIG_CS75XX_HW_ACCEL_L2TP_CTRL)
/* file handler for cs_ne_ipsec_offload_mode*/
static int cs_proc_node_vpn_offload_mode_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS_VPN_OFFLOAD_MODE_HELP_MSG,
...
static int cs_proc_node_udp_offload_after_established_read(struct seq_file *m, void *v)
{
	seq_printf(m, CS752X_NE_UDP_OFFLOAD_AFTER_ESTABLISHED,
...
static int cs_proc_node_wireless_mode_read(struct seq_file *m, void *v)
{
	seq_printf(m, " cs_hw_accel_wireless_mode = %d support %d ssid\n",
...
#define CS752X_DEFINE_RO_PROC_OPS(x) _CS752X_DEFINE_PROC_OPS(x, x##_read, NULL)
#define CS752X_DEFINE_RW_PROC_OPS(x) _CS752X_DEFINE_PROC_OPS(x, x##_read, x##_write)


#ifdef CONFIG_CS752X_HW_ACCELERATION
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_kernel_mod_hw_accel_enable)
#endif

#ifdef CONFIG_CS752X_FASTNET
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_kernel_mod_fastnet_enable)
#endif
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_hash_hw_entry)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_core_hmu_tbl)
CS752X_DEFINE_RO_PROC_OPS(cs_proc_node_core_vtbl)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_hash_timer_period)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_hash_default_lifetime)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_tcp_bypass_port)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_udp_bypass_port)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_expected_bypass_port)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_nf_drop_check_try)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_nf_drop_check_life)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_nf_drop_check_table)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_tcp_priority_port)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_udp_priority_port)
#ifdef CONFIG_CS752X_HW_ACCELERATION_IPSEC
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_ipsec_sadb_table)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_ipsec_pe_sadb_table)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_ipsec_offload_mode)
#endif
#ifdef CONFIG_CS75XX_HW_ACCEL_IPLIP
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_iplip_pe_cmd)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_iplip_kernel_cmd)
#endif
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_udp_offload_after_established)
#if defined(CONFIG_CS75XX_HW_ACCEL_IPSEC_CTRL) ||\
		defined(CONFIG_CS75XX_HW_ACCEL_L2TP_CTRL)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_vpn_offload_mode)
#endif

#ifdef CONFIG_CS75XX_HW_ACCEL_WIRELESS
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_wireless_mode)
#endif
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_allow_localin_port)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_node_localin_udp_drop_statistic)
CS752X_DEFINE_RW_PROC_OPS(cs_proc_acl_add)


int cs_proc_node_add(char *name, const struct file_operations *fops,
		struct proc_dir_entry *parent)
{
	struct proc_dir_entry *node;

	node = proc_create(name, S_IRUGO | S_IWUGO, parent, fops);
	if (!node) {
