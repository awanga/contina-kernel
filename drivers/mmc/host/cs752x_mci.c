...
static int dw_mci_pltfm_probe(struct platform_device *pdev)
{
	...
	if (!host->regs)
		goto err_free;
	printk( "sd mapping 0x%08x host->irq(%d) \n", host->regs,host->irq);
...
static int __init cs752x_mci_init(void)
{
	int status;
...
static void __exit cs752x_mci_exit(void)
{
	platform_driver_unregister(&cs752x_sd_driver);
}

module_init(cs752x_mci_init);
module_exit(cs752x_mci_exit);
