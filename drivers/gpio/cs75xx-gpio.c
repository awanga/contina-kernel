...
static void cs75xx_gpio_irq_handler(struct irq_desc *desc)
{
	...
	for (; irq_stat != 0; irq_stat >>= 1, gpio_irq_no++) {
		...
		irq_desc[gpio_irq_no].handle_irq(&irq_desc[gpio_irq_no]);
	}
}
...
static int cs75xx_gpio_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < GPIO_BANK_NUM; i++) {
		...
		irq_set_chained_handler_and_data(cs75xx_irq_gpio[i],
			cs75xx_gpio_irq_handler, (void *)i);
	}
...
static int cs75xx_gpio_remove(struct platform_device *pdev)
{
	...
	gpiochip_remove(&cs75xx_gpio_chip);
