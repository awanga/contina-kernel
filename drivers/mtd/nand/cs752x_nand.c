...
#define CS752X_MAX_PAGESIZE    8192
...
static int cs752x_ooblayout_ecc_lp_bch(struct mtd_info *mtd, int section,
				struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	const unsigned ecc_size = 13, spare_size = 2;

	oobregion->offset = (ecc_size + spare_size) * section) +
			    spare_size;
	oobregion->length = ecc_size;
	return 0;
}

static int cs752x__ooblayout_free_lp_bch(struct mtd_info *mtd, int section,
				 struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	const unsigned ecc_size = 13, spare_size = 2;

	oobregion->offset = section * (ecc_size + spare_size);
	oobregion->length = spare_size;
	return 0;
}

static const struct mtd_ooblayout_ops cs752x_ooblayout_lp_bch_ops = {
	.ecc = cs752x_ooblayout_ecc_lp_bch,
	.free = cs752x_ooblayout_free_lp_bch,
};
#endif
 ...
 */
static int cs752x_ooblayout_ecc_lp_hamming(struct mtd_info *mtd, int section,
					 struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	...
	case 128:
		...
		break;
	case 256:
		oobregion->offset = 160;
		break;
	case 512:
		oobregion->offset = 320;
		break;
	default:
		return -EINVAL;
	}
...
static int cs752x_ooblayout_free_lp_hamming(struct mtd_info *mtd, int section,
					  struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct nand_ecc_ctrl *ecc = &chip->ecc;
...
const struct mtd_ooblayout_ops cs752x_ooblayout_lp_hamming_ops = {
	.ecc = cs752x_ooblayout_ecc_lp_hamming,
	.free = cs752x_ooblayout_free_lp_hamming,
};
...
static int cs752x_nand_block_checkbad(struct mtd_info *mtd, loff_t ofs, int allowbbt);
...
#ifdef	NAND_ECC_TEST
unsigned char tw[CS752X_MAX_PAGESIZE]__attribute__((aligned(16))) ,  tr[CS752X_MAX_PAGESIZE]__attribute__((aligned(16))); ;
...
static int cs752x_nand_block_isbad(struct mtd_info *mtd, loff_t offs)
{
	...
	rc = cs752x_nand_block_checkbad(mtd, offs, 0);
 ...
 */
static int cs752x_nand_read_oob_std(struct mtd_info *mtd, struct nand_chip *chip, int page)
{
	int i;
...
#ifdef	NAND_ECC_TEST
	memset(tw, 0, CS752X_MAX_PAGESIZE);
...
out_copy_done:
	...
	if (((unsigned int)addr) ==
	    ((unsigned int)(cs752x_host->io_base) + SZ_128M)) {
		cs752x_nand_read_oob_std(mtd, chip, page);
	} else {
		for (i = 0; i < (mtd->oobsize / 4); i++) {
			*(pread + i) = *((unsigned int *)addr + i);
		}
...
{
	u32 i, j;
	u32 *eccpos = chip->buffers->ecccalc;
	int eccsteps = chip->ecc.steps;
...
#ifdef	NAND_ECC_TEST
	memset(tr , 0, CS752X_MAX_PAGESIZE);
...
int cs752x_nand_scan_tail(struct mtd_info *mtd)
{
	...
	if (!mtd->ooblayout) {
		...
		if (mtd->oobsize == 16) {
			//mtd_set_ooblayout(mtd, &cs752x_ooblayout_sp_bch_ops);
		...
		if ((mtd->oobsize == 8) || (mtd->oobsize == 16)) {
			//mtd_set_ooblayout(mtd, &cs752x_ooblayout_sp_hamming_ops);
			mtd_set_ooblayout(mtd, &nand_ooblayout_sp_ops);
		} else
#endif
		{
#ifdef CONFIG_CS752X_NAND_ECC_HW_BCH
			mtd_set_ooblayout(mtd, &cs752x_ooblayout_lp_bch_ops);
#elif defined( CONFIG_CS752X_NAND_ECC_HW_HAMMING_512 ) ||  defined( CONFIG_CS752X_NAND_ECC_HW_HAMMING_256 )
			mtd_set_ooblayout(mtd, &cs752x_ooblayout_lp_hamming_ops);
#else
...
static int cs752x_nand_verify_buf(struct mtd_info *mtd, const uint8_t *buf, int len)
{
	...
	chip->ecc.read_page(mtd, chip, chip->buffers->databuf, mtd->oob ? 1 : 0, page);
 ...
 */
static int cs752x_nand_block_bad(struct mtd_info *mtd, loff_t ofs)
{
...
static uint8_t cs752x_nand_read_byte(struct mtd_info *mtd)
{
	...
	cs752x_nand_read_page_raw(mtd, chip, chip->buffers->databuf, 1, page);
 ...
 */
static int cs752x_nand_remove(struct platform_device *pdev)
{
