		...
		enum ip_conntrack_info ctinfo)
{
	...
	if ((cmd.l3num == nf_ct_l3num(ct)) &&
		...
		/* FK: do not allow non matching ip addr for PORT and EPRT
		   convert command to an illegal none so client sees a failure
		*/
		if (dir == IP_CT_DIR_ORIGINAL
		&&  nf_nat_mangle_tcp_packet(skb, ct, ctinfo, 0, 4, "NoGo", 4)) {
			ret = NF_ACCEPT;
			goto out_put_expect;
		}
