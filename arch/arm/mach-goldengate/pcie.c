...
void g2_pcie_msi_clear(int rc_number)
{
	...
	volatile void __iomem *reg_offset;
	...
	reg_offset = (volatile void __iomem *)
		(pp->base_addr + PCIE_PORT_MSI_CONTROLLER_INT0_STATUS);
	...
	reg_offset = (volatile void __iomem *)
		(PCIE_GLBL_INTERRUPT_0 + rc_number * PCIE_GLBL_offset);
...
static void mask_intx_irq(struct irq_data *irqd)
{
	...
	int temp;
	temp = readl((volatile void __iomem *)
...
//#endif
	...
	temp &= ~(INTD_ASSERT_EN | INTC_ASSERT_EN | INTB_ASSERT_EN | INTA_ASSERT_EN);
        writel(temp, (volatile void __iomem *)
		(PCIE_GLBL_INTERRUPT_ENABLE_0 + (pp->rc_number * PCIE_GLBL_offset)));
}
...
//debug_Aaron
	...
	irq = irq_alloc_desc_from(pcie_find_irq_by_pdev(pdev), -1);
...
//printk("%s: bus->self->hdr_type=0x%x, bus->number=%d\r\n", __func__, bus->self->hdr_type, bus->number);
	...
	base_addr = (volatile void __iomem *)pp->base_addr;
...
void g2_pcie_phy_init(int rc_number)
{
	...
	volatile void __iomem *reg_offset;
 	...
 	temp = readl((volatile void __iomem *)GLOBAL_GLOBAL_CONFIG);
...
#endif
 	writel(temp, (volatile void __iomem *)GLOBAL_GLOBAL_CONFIG);

 	//2. app_ltssm_enable set 1
	reg_offset = (volatile void __iomem *)
		(PCIE_SATA_PCIE_GLBL_CORE_CONFIG_REG + rc_number * PCIE_GLBL_offset);
...
static int g2_pcie_rd_conf(struct pci_bus *bus, u32 devfn, int where, int size, u32 *val)
{
	...
	struct pcie_port *pp;
	volatile void __iomem *base_addr;
	volatile void __iomem *cnf_base_addr;
 ...
 //debug_Aaron, check where the request come from, device or RC ?
	...
	if (PCIe_bridge_flag == 1)
	{
   		...
   		if(pp != NULL) //comes from RCs
   		{
    		...
    		switch (size)
      		{
       			case 1:
              		*val = readb((volatile void __iomem *)(base_addr + where));
              		break;
       			case 2:
               		*val = readw((volatile void __iomem *)(base_addr + where));
               		break;
       			case 4:
...
#endif
               			*val = readl((volatile void __iomem *)(base_addr + where));
               		break;
       			default:
               		*val = 0;
               		break;
	...
	cnf_base_addr = (volatile void __iomem *)pp->cnf_base_addr;
    switch (size)
    {
    	case 1:
...
void g2_pcie_register_init(int rc_number)
{
	...
	writel(GOLDENGATE_PCIE0_BASE + rc_number * GOLDENGATE_PCI_MEM_SIZE,
		(volatile void __iomem *)(PCIE_SATA_PCIE_GLBL_PCIE_CONTR_CFG_START_ADDR + rc_number * PCIE_GLBL_offset));

	//2. Setup PCIe Configuration end address (PCIE_SATA_PCIE_GLBL_PCIE_CONTR_CFG_END_ADDR)
	writel(GOLDENGATE_PCIE0_BASE+0x0FFF+rc_number * GOLDENGATE_PCI_MEM_SIZE,
		(volatile void __iomem *)(PCIE_SATA_PCIE_GLBL_PCIE_CONTR_CFG_END_ADDR + rc_number * PCIE_GLBL_offset));
...
//	readl(PCIE_SATA_PCIE_GLBL_PCIE_CONTR_CFG_END_ADDR + rc_number * PCIE_GLBL_offset);
	for (i=0; i<256; i++) {
		value = readl((volatile void __iomem *)
			(PCIE_SATA_PCIE_GLBL_PCIE_CONTR_CFG_END_ADDR + rc_number * PCIE_GLBL_offset));
...
static void g2_pcie_enable_interrupt(int rc_number)
{
	//Enable Pcie Sata Pcie Glbl Interrupt Enable 0 Register, 0xf00a0004 and 0xf00a0404
    writel(MSI_CTR_INT_EN | INTD_ASSERT_EN | INTC_ASSERT_EN | INTB_ASSERT_EN | INTA_ASSERT_EN,
            (volatile void __iomem *)(PCIE_GLBL_INTERRUPT_ENABLE_0 + (rc_number * PCIE_GLBL_offset)));
...
static void g2_pcie_read_err_counts(int port_number)
{
        ...
        err_count = readl((volatile void __iomem *)
		(PCIE_SATA_PCIE_GLBL_PHY_RX_10B_8B_DEC_ERR_CNT + port_number * 0x400));
        printk("%s: PCIE_SATA_PCIE_GLBL_PHY_RX_10B_8B_DEC_ERR_CNT=%d\r\n", __func__, err_count);

        err_count = readl((volatile void __iomem *)
		(PCIE_SATA_PCIE_GLBL_PHY_RX_DISPARITY_ERR_CNT  + port_number * 0x400));
        ...
        {
                err_count = readl((volatile void __iomem *)
			(PCIE_SATA_PCIE_GLBL_RX_RCVD_TS1_ORDER_SET_CNT  + port_number * 0x400));
...
static irqreturn_t g2_pcie_irq(int irq, void *devid)
{
	...
	status = readl((volatile void __iomem *)
		(PCIE_GLBL_INTERRUPT_0 + (irq - IRQ_PCIE0) * PCIE_GLBL_offset));
	writel(status, (volatile void __iomem *)
		(PCIE_GLBL_INTERRUPT_0 + (irq - IRQ_PCIE0) * PCIE_GLBL_offset));
...
g2_pcie_scan_bus(int nr, struct pci_sys_data *sys)
{
    ...
    writew(0x0007, (volatile void __iomem *)(base_addr + 0x4));
    writel(0x070280, (volatile void __iomem *)(base_addr + 0x71c));

    //Add For Debug "remote max request size"
    temp = readb(base_addr + 0x818);
    writeb(0x2, (volatile void __iomem *)(base_addr + 0x818));
...
static int __init g2_pcie_map_irq(const struct pci_dev *dev, u8 slot, u8 pin)
{
	struct pcie_port *pp;
...
//#endif
			irq_clear_status_flags(g2_pcie_map_no, IRQ_NOREQUEST);
		}
...
static void g2_pcie_reset_device(unsigned int gpio)
{
	...
	volatile void __iomem *gpio_mux;
	volatile void __iomem *gpio_out;
	volatile void __iomem *gpio_cfg;
	unsigned int temp;

	gpio_num = gpio / 32;
	gpio_pin = gpio % 32;

	gpio_mux = (volatile void __iomem *)(GLOBAL_GPIO_MUX_0 + gpio_num * 4);
	gpio_out = (volatile void __iomem *)(PER_GPIO0_OUT + gpio_num * 0x20);
	gpio_cfg = (volatile void __iomem *)(PER_GPIO0_CFG + gpio_num * 0x20);
...
static int g2_pcie_remove(struct platform_device *pdev)
{
	//debug_Aaron
	int i;
...
static int __init g2_pcie_init(void)
{
    int temp;
